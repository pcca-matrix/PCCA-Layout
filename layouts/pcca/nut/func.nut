...
function ret_wheel( offset ){
    ...
    if(fe.game_info(Info.Emulator) == "@"){
        ...
        m = medias_path + fe.game_info(Info.Emulator) + "/Images/Wheel/"+fe.game_info(Info.Name, offset);
    }
    return m;
}
...
function ret_snap(){
    ...
    if(fe.game_info(Info.Emulator) == "@"){
        ...
        m = medias_path + fe.game_info(Info.Emulator) + "/Video/"+fe.game_info(Info.Name) + ".mp4";
    }
    return m;
}
